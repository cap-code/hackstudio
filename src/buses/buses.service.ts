...
export class BusesService {
    ...
    async insertbustrip(startt:string,endt:string,startl:any,endl:any){
      const new_prod=new this.busetripmodel({start_time:startt,ending_time:endl,starting_location:startl,ending_location:endl});
     ...
     async  insertbus(busid:string,name:string,regid:string){
        const new_prod=new this.busesmodel({busid:busid,name:name,reg_id:regid});
        const result= await new_prod.save();
        return result.id as string;
      }
      async getbus(){
          const bus=await this.busesmodel.find.exec();
          return bus;
      }
     } 
